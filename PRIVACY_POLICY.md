...
Our extension operates completely locally on your device. We do not collect, transmit, or store any personal data, browsing history, or user information on remote servers.

### Local Storage
The extension uses your browser's local storage (`chrome.storage`) to save:
- Your application settings (e.g., background change interval).
- Local image paths or URLs you explicitly chose to use as backgrounds.

This data remains strictly on your local device and is sync-enabled only via Google Chrome's built-in synchronization feature if you are logged into your Google Account and have syncing enabled.

## Permissions
The extension requests the following permissions for specific functionalities:
- **`storage`**: To save your preferences and locally added images as described above.
